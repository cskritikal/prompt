...
> 	- [[#1.3 Self-Serve Enrichment Before Escalating|1.3 Self-Serve Enrichment]] · [[#1.4 Defer to Analyst Override (No Re-Arguing)|1.4 Analyst Override]] · [[#1.5 Exhaustive Deep Investigation & Zero Non-Data|1.5 Deep Investigation]]
...
### 1.5 Exhaustive Deep Investigation & Zero Non-Data
#workflow/investigation #taps/execution #zero-hallucination #evidence-first

> [!warning] Trigger Condition
> The agent performs a superficial analysis, skips available enrichment TAPs/tools, hallucinates query results, or fills the output with speculative commentary/non-data.
...
### 2.7 No Fabricated Tool Execution / Honest Tool Auditing
#grounding/tool-honesty #anti-hallucination #zero-simulation

> [!warning] Trigger Condition
> The agent claims a tool, SIEM, or table was queried and returned no results (e.g. "Splunk returned no matching telemetry", "Defender search returned empty") when no actual tool query was executed in session, or simulates tool output.

> [!quote] Copyable Prompt Snippet
> ```markdown
> Zero tool hallucinations: Never claim a tool, SIEM, or query was executed or returned empty (e.g. "Splunk returned no matching telemetry", "EDR search returned 0 events") unless you actually executed that tool call in this session. If a source was not queried or is not callable in your session, state `Gap: [Specific Source] unavailable to verify [exact fact]` or omit the Context line entirely. Re-evaluate using ONLY telemetry actually present in the session.
> ```
...
### 5.2 Remove Generic / Forbidden / SOC-Side Recommendations
#formatting/recommendations #technical-actions #client-side-only

> [!warning] Trigger Condition
> The agent writes generic actions ("notify customer", "monitor", "investigate further"), internal SOC-side actions ("SOC will continue to monitor", "SOC to review SIEM logs", "SOC is hunting across tenants", "Tune detection rule", "Internal SOC follow-up"), conditional hedging ("if unrecognized", "if recipient entered credentials", "after confirmed compromise"), tautological label-colon repetition (`Reset user: Reset credentials...`), lazy meta-IOC references ("the identified sender addresses"), or asks the client to investigate ("Confirm whether subscriptions were initiated").
...
> * Revoke active session tokens and reset credentials for user `DOMAIN\m.banker` via Entra ID Admin Center
...
> * Hunt endpoint fleet for hash `8f9a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a2b3c4d5e6f7a8b9c0d1e2f3a`
> ```
> 1. **Hard budget:** ≤5 lines total.
> 2. **Direct Imperative Action:** Each line MUST start with an imperative action verb (`Isolate`, `Quarantine`, `Block`, `Purge`, `Reset`, `Revoke`, `Verify`, `Hunt`, `Inspect`, `Terminate`, `Remove`, `Detach`).
> 3. **Client-Side Actions Only (Zero SOC-Side Actions):** All recommendations must be direct technical actions for the customer/client IT or security team to execute in their environment (`Isolate host`, `Reset user password`, `Block IP`, `Purge email`). FORBIDDEN: internal SOC-side tasks or workflow commentary (`SOC will monitor...`, `SOC team will hunt...`, `SOC to review SIEM logs`, `Tune detection rules`, `Escalate to Tier 2`, `SOC follow-up`). Recommendations are instructions FOR the client, not a task list for the SOC.
> 4. **Zero Conditional Hedging (No "If-Disease"):** FORBIDDEN: `if [condition]`, `after [event]`, `in case of`, `if unrecognized`, `if confirmed`, `if credentials entered`. Give definitive, unconditional technical instructions calibrated to the priority. (Only allowed exception: `* If this was expected, the alert may be closed with a comment.` on Low/benign).
> 5. **Zero Tautological Colon Stutter:** FORBIDDEN: `Reset user: Reset credentials...`, `Revoke user: Revoke active sessions...`, `Quarantine delivered messages: Remove 3 messages...`. State the direct command without pseudo-header repetition.
> 6. **Zero Lazy Meta-IOC References:** FORBIDDEN: `"the identified senders/IPs"`, `"related links"`, `"similar emails"`, `"any suspicious activity"`. Name explicit discrete IOC values, hashes, domains, or query parameters.
> 7. **Actionable Verification Only (No Delegated Triage):** FORBIDDEN: asking customer to investigate (e.g. `"Confirm whether subscriptions were initiated"`). State specific high-risk operational events to verify (e.g. `"Verify with user whether unauthorized password resets or wire transfers occurred during the flood window"`).
> 8. **Calibrate to Threat Certainty & Severity:**
...
> 
> * **Verdict:** MUST be strictly `Malicious`, `Suspicious`, or `Benign`. **FORBIDDEN:** `Inconclusive`, `Inconclusive - Evidence Gap`, `Evidence Gap`, `Unknown`, `Undetermined`, `Informational`. (Record telemetry gaps strictly as `Gap: [Source] unavailable to verify [fact]` inside Context with Confidence set to `Unconfirmed` or `Indicated`—never as a verdict name).
...
> `DISPOSITION: [Verdict] · [Confidence] · [Priority] · ROUTE [1 Escalation / 2 Orchestration / 3 Manual Closure]`
> 
> * **Verdict:** MUST be strictly `Malicious`, `Suspicious`, or `Benign`. **FORBIDDEN:** `Inconclusive`, `Inconclusive - Evidence Gap`, `Evidence Gap`, `Unknown`, `Undetermined`, `Informational`. (Record evidence gaps strictly as `Gap: [Source] unavailable to verify [fact]` inside Context with Confidence set to `Unconfirmed` or `Indicated`).
> * **Confidence:** MUST be strictly `Confirmed`, `Indicated`, or `Unconfirmed`.
> * **Priority:** MUST be strictly `Filter-Close`, `Low`, `Medium`, or `High`.
> * **Route:** MUST be strictly `ROUTE 1 Escalation`, `ROUTE 2 Orchestration`, or `ROUTE 3 Manual Closure`.
> 
